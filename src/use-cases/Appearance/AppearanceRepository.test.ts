...
describe('remembered appearance', () => {
  it('starts in dark mode with the neutral theme when nothing was chosen before', () => {
    const repository = appearanceRepositoryFor(inMemoryStorage)

    expect([repository.loadMode(), repository.loadTheme()]).toEqual(['dark', 'neutral'])
  })
  ...
  it('falls back to the defaults when storage cannot even be opened', () => {
    ...
    expect(repository.loadTheme()).toBe('neutral')
  })
})
