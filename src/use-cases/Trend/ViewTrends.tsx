    ...
    <>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Stock, flow, and whether the movement is trustworthy.</p>
        </div>
        ...
        {trends.hasTrend && (
          <div className="grid gap-6 lg:grid-cols-2">
            <SectionCard title="Open-to-Work rate" description="Daily rate with a 7-day moving average across actual scans">
              <LineTrendChart data={trends.points} xKey="scanDate" series={openRateSeries} formatX={formatDateTick} formatY={formatRateTick} />
            </SectionCard>
            ...
            <SectionCard title="Raw vs matched cohort" description="Matched = only people classified in both this scan and the one before">
              <LineTrendChart data={trends.points} xKey="scanDate" series={matchedRateSeries} formatX={formatDateTick} formatY={formatRateTick} />
            </SectionCard>
            <SectionCard title="Entry vs removal" description="People whose public frame appeared or disappeared">
              <BarComparisonChart data={trends.points} xKey="scanDate" series={openStatusChangeSeries} formatX={formatDateTick} />
            </SectionCard>
            <SectionCard title="Net Open-to-Work flow" description="Positive: more entries than exits. Negative: more exits than entries.">
              <LineTrendChart data={trends.points} xKey="scanDate" series={netFlowSeries} formatX={formatDateTick} formatY={formatCountTick} showZeroLine />
            </SectionCard>
            <SectionCard
            ...
            >
              <LineTrendChart data={trends.points} xKey="scanDate" series={transitionRateSeries} formatX={formatDateTick} formatY={formatRateTick} />
            </SectionCard>
